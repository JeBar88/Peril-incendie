...
perte <- don$Estimated_Dollar_Loss[!is.na(don$Estimated_Dollar_Loss) & 
                                     don$Estimated_Dollar_Loss > 0 & 
                                     (don$Building_Status == "03 - Under Construction" |
                                     don$Building_Status == "02 - Under Renovation")]
...
xi <- mle.PaG[2]
s <- mle.PaG[1]
cbind("alpha"=1/xi, "lambda"=s/xi, xi, s) # alpha = 1.431 # alpha = 1.446116

# Comparaison avec fonction de réparition empirique
Fx.PaG <- function(x, u) Fn(u) + (1-Fn(u)) * pgpd(x, xi, u, s)

plot(Fn, ylim=c(0.70, 1), lwd = 2, main = "Fonction de répartition empirique comparé avec Pareto généralisée")
