...
testln.pg <- function(k, u, par){
  m <- par[1] ; r <- par[2] ; xi <- par[3]
  sigma <- par[4] ; p <- par[5]
  qlnorm(min(k * plnorm(u, m, r)/p, 0.999), m, r) * I(k <= p) + qgpd((k - p)/(1-p), xi, u, sigma) * I(k > p)
}
...
tr.ks <- function(data, u){
  ...
  for(i in 1:length(u)){
    ...
    test <- ks.test(data[data > u[i]], function(x) pgpd(x, param[i, 2], u[i], param[i, 1]))
    stat[i] <- test[[1]]
    p[i] <- test[[2]]
  }
...
### Base de données ###
data("frecomfire")
perte <- frecomfire$ClaimCost2007
...
# QQ plot Pareto
ParetoQQ(perte, xlim=c(0, 10))
abline(h=log(1146), col = "red")
...
Hill(perte, plot = T, k=T, xlim = c(0, 9000))
...
plot(Fn, 
     ...
     cex.lab=1.3, cex.axis=1.3, cex.main=1.3, lwd =2)
...
xi <- mle.PaG[2]
s <- mle.PaG[1]
cbind("alpha"=1/xi, xi, s) # alpha = 2.0025

# Comparaison avec fonction de réparition empirique
Fx.PaG <- function(x, u) Fn(u) + (1-Fn(u)) * pgpd(x, xi, u, s)

plot(Fn, ylim=c(0.90, 1), xlim=c(0, 1e6),
